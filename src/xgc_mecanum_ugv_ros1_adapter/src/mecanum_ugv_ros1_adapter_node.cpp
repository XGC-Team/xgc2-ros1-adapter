...
bool resolveOperation(const std::string &profile_id,
  ...
  *known = false;
  ...
  if (channels == nullptr || channel_count == 0u) {
    *known = true;
    *error = "installed Mecanum operation profile is unavailable";
    return false;
  }
...
public:
  ...
  MecanumUgvRos1AdapterNode(const MecanumUgvRos1AdapterNode &) = delete;
  MecanumUgvRos1AdapterNode &
  operator=(const MecanumUgvRos1AdapterNode &) = delete;
...
private:
  ...
  bool validateRobotConfig(const xgc2_ros1_robot_adapter::RobotConfig &robot,
    ...
    if (robot.profile_id != contract::kProfileId) {
      *error = "Mecanum Adapter received unsupported profile " + robot.profile_id;
      return false;
    }
    ...
    for (const auto &channel : robot.channels) {
      ...
      if (!contract::channelMetadata(robot.profile_id, channel.channel_id,
                                     &metadata)) {
        *error = "Mecanum robot " + robot.robot_id +
      ...
      if ((metadata.kind == contract::ChannelKind::kOperation) !=
        ...
        *error = "Mecanum operation timing metadata is inconsistent";
        return false;
      }
    }
    return true;
  ...
  bool applyInstanceSpec(const xgc::adapter::v1::AdapterInstanceSpec &spec,
    ...
    for (const auto &robot : candidate.robots) {
      ...
      if (!namespaces.insert(robot.parameters.at("namespace")).second) {
        *error = "multiple Mecanum robots share ROS namespace " +
  ...
  xgc2::adapter_runtime::SourceOpenDecision handleSourceOpen(
    ...
    if (context.capability_id() !=
      ...
      return rejectSource(xgc::adapter::v1::ERROR_CLASS_REJECTED,
                          ...
                          "unsupported Mecanum telemetry source endpoint");
    }
  ...
  void handleSourceClosed(const xgc::adapter::v1::SourceOpenRequest &request,
    ...
    if (runtime)
      runtime->Stop();
    if (matched) {
      ROS_WARN_STREAM("Mecanum telemetry source "
  ...
  xgc2::adapter_runtime::OperationResult handleCommand(
    ...
    if (!motion) {
      return rejectOperation("command-disabled",
                             "Mecanum motion control is disabled by robot spec");
    }
    ...
    const bool set_intent = processor == "mecanum-ugv.set-motion-intent";
    const bool release_intent =
        processor == "mecanum-ugv.release-motion-intent";
    if (!set_intent && !release_intent) {
      return xgc2::adapter_runtime::OperationResult::Failure(
          xgc::adapter::v1::ERROR_CLASS_PERMANENT,
          "operation-contract-invalid",
          "installed Mecanum operation processor has no native binding");
    }
    ...
    if (!motion) {
      return rejectOperation("command-disabled",
                             "Mecanum remote control is disabled by robot spec");
    }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    ROS_FATAL_STREAM(
        "Mecanum robot Adapter startup failed: " << exception.what());
    return 1;
  }
  return 0;
}
