...
#include "xgc_unitree_b2_ros1_adapter/telemetry_batch.hpp"
...
public:
  UnitreeB2Ros1AdapterNode(ros::NodeHandle node_handle,
    ...
    periodic_timer_ = node_handle_.createWallTimer(
        ros::WallDuration(0.1), &UnitreeB2Ros1AdapterNode::periodicTimer, this);
  }

  ~UnitreeB2Ros1AdapterNode() { Shutdown(); }
  ...
  UnitreeB2Ros1AdapterNode(const UnitreeB2Ros1AdapterNode &) = delete;
  UnitreeB2Ros1AdapterNode &
  operator=(const UnitreeB2Ros1AdapterNode &) = delete;
...
private:
  ...
  bool validateRobotConfig(const xgc2_ros1_robot_adapter::RobotConfig &robot,
    ...
    if (robot.profile_id != contract::kProfileId) {
      *error = "B2 Adapter received unsupported profile " + robot.profile_id;
      return false;
    }
    ...
    if (robot.parameters.at("wire_transport") != "tcp") {
    ...
    for (const auto &channel : baseline)
      if (enabled.count(channel) == 0u) {
        *error = "B2 baseline channel must be enabled: " + channel;
        return false;
      }
    return true;
  }
  ...
  xgc2::adapter_runtime::SourceOpenDecision handleSourceOpen(
    ...
    if (context.capability_id() !=
      ...
      return rejectSource(xgc::adapter::v1::ERROR_CLASS_REJECTED,
                          ...
                          "unsupported B2 telemetry source endpoint");
    }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // Stop the Runtime client while ROS transport is still alive. The
    // destructor remains an idempotent fallback.
    node.Shutdown();
