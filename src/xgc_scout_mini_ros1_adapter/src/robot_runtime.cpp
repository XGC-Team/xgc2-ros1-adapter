...
bool validateNativeProfileContract(std::string *error) {
    ...
    if (binding.observes_channels) {
      ...
      if (localization_projection) {
        ...
        if (projection_output == nullptr ||
          ...
          return fail(error, "Scout localization output binding drifted");
        }
      }
      const auto *endpoint = contract::channelEndpoint(
  ...
  if (!contract::channelMetadata(contract::kProfileId,
      ...
      std::string(release.processor) != "scout-mini.release-motion-intent" ||
      ...
      std::string(release_input.type_name) !=
          "xgc.semantic.common.v1.RemoteControlIntentRequest") {
    return fail(error, "Scout motion-intent-release operation binding drifted");
  }
  ...
  if (release_endpoint == nullptr ||
    ...
    return fail(error, "Scout motion-intent-release output topic binding drifted");
  }
